...
const char* clientName = "";  // setup 함수에서 자동생성 중복되면 깨짐 100글자 이상도 깨짐
...
void setup(void) {
  ...
  client.setServer(mqtt_server, 1883); //mqtt설정
...
void reconnect() {
  ...
  while (!client.connected()) { //클라이언트 연결
